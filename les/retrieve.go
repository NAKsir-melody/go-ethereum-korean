...
// cancelled.
// retrieve(검색) 함수는 필요하다면 여러 피어에게 요청을 보내고 
// 딜리버 함수를 통해 배달되 검증자 콜백을 통해 성공적으로 검증된 응답을 기다린다
// 이 함수는 컨텍스트가 취소되거나, 유효한 응답이 반횐될 경우 리턴된다
...
// sendReq함수는 요청에 대해 유효한 응답을 검색 적당한 피어로 부터 수신하기 위해 노력한다
...
// tryRequest함수는 요청을 새로운 피어에게 전송하고 성공하거나 타임아웃될때까지 대기한다
// 또 적절한 피어 이벤트 메시지를 요청한 이벤트채널로 전달한다
