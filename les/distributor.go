...
// 외부 피어에 대한 다수의 odr 요청을 관리하기 위한 분산처리자
package les
...
// distReq는 분산자에의해 사용되는 요청의 추상적 개념이며 3개의 콜백으로 구성된다
// -getCost는 트렌젝션을 주어진 피어에게 전달할때 최대 예측 가격을 반환한다
// -canSend함수는 마약 서비 피어가 요청을 처리하거 적당함을 알려준다
// -request 함수는 주어진 피어에게 요청을 전달할 준비를 하고 실제 전송함수를 반환한다. 
// 요청순서는 보전적이여야하지만 콜백은 전송이 끝날때까지 블록이 되면 안된다 
// 대신 반환된 함수는 피어의 전송큐로 들어간다
type distReq struct {
