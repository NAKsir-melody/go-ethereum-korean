...
// light패키지는 이더리움 라이트 클라이언트의 상태와 체인 오브젝트에 대한 
// on-demand retrieval (사용자 요구에 따른 검색) 을 구현한다
package les
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
// 함수는 odr LES 네트워크로 부터 온 ODR요청응답메시지를 처리하고 
// true를 리턴하고,결과를 메모리에 저장한다
...
// 계약코드 검색을 위해 사용되는 노드데이터에 대한 odr 요청타입
...
// Request sends an ODR request to the LES network (implementation of LesOdrRequest)
// Request는 odr request를 LES네트워크로 전송한다
...
// Request sends an ODR request to the LES network (implementation of LesOdrRequest)
// Request는 Odr request를 LES network로 전달할때 사용된다
