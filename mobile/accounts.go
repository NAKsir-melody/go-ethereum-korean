...
// 모바일 플랫폼상에서 어카운트 패키지부터 클라이언트 사이드의 키 관리 시스템까지를 
// 포함하는 모든 래퍼들
...
// SignHash는 주어진 해시를 위한 ECDSA 서명을 계산한다. 생성된 서명은 V는 이나 1일때 R.S.V포멧에 저장된다
...
// SignTx는 요구된 계정으로 주어진 트렌젝션에 서명한다
...
// SignHashPassphrase는 주어진 암호로 해독가능한 주어진 계정의 개인키가 매칭될경우 해시에 서명한다
// 생성된 서명은 V는 이나 1일때 R.S.V포멧에 저장된다
...
// SignTxPassphrase는 주어진 암호로 해독가능한 주어진 계정의 개인키가 매칭될경우 트렌젝션에 서명한다
...
// Lock은 주어진 계정의 개인키를 메모리로 부터 제거한다
...
// TimeUnlock은 계정을 주어진 암호로 언락한다. 계정은 특정기간(나노초)동안 언락된 상태로 유지된다.
// 타임아웃이 0일경우 계정은 프로그램이 끝날때까지 언락된다. 계정은 특정 키파일에 맞아야 한다
// 만약 계정의 주소가 이미 언락되었다면, 이함수는 언락시간을 늘리거나 줄인다.
// 만약 주소가 무한정으로 언락되었다면 타임아웃은 수정되지 않는다
...
// NewAccount는 새로운 키를 생성하고 키 디렉토리로 저장한다
// 저장된 것을 주어진 암호로 암호화한다
