...
// addTrustedCheckpoint함수는 신뢰된 체크포인트를 블록체인에 추가한다
...
// loadLastState 함수는 마지막으로 알려진 체인상태를 db로 부터 읽는다
// 이함수는 체인메니저의 뮤택스가 잡혀있다고 가정한다
...
// State함수는 현재 해드블록을 기반으로 새로운 변환가능한 상태를 반환한다
...
// GetBody함수는 블록바디(트렌젝션과 엉클들)을 DB나 ODR을 통해 반환하고 캐싱한다
...
// GetBodyRLP함수는 블록바디(트렌젝션과 엉클들)을 DB나 ODR을 통해 반환하고 캐싱한다
...
// it if present.
// HasBlock함수는 블록이 DB에 완벽히 존재하는지를 체크하며 캐싱한다
...
// caching it if found.
// GetBlock함수는 db나 ODR서비스를 통해 해시와 번호로 블록을 반환하고 
// 찾아졌을 경우 캐싱한다
...
// caching it if found.
// GetBlockByHash함수는 db나 ODR서비스를 통해 해시로 블록을 반환하고 
// 찾아졌을 경우 캐싱한다
...
// GetBlcokByNumber함수는 블럭넘버로 DB나 ODR서비스로부터 블록을 반환하고 찾아졌을 경우 해싱한다
...
// postChainEvents함수는 체인 삽입에 의해 생성된 이벤트들을 반복하면서 
// event feed로 전달한다
