...
// 신뢰된 체크포인트는 적절한 섹션 인덱스와 헤드해시와 연관된 후처리된 트라이의 루트(chtd와 블룸trie)를 나타낸다
// 이것은 설정된 체크포인트로부터 오래된 헤더와 로그를 안전하게 접근할수 있을때 까지 라이트 싱크를 스타트하기위해 사용되며
// 전체 헤더 체인을 다운 받는것을 방지한다
...
// trustedCheckpoints 는 체인에 각각 속한 제네시스 해시에 대해 
// 알려진 체크포인트와 연결된다.
...
// ChtNode는 RLP 인코딩된 형태의 캐노니컬 해시 트라이를 저장한다
type ChtNode struct {
	Hash common.Hash
	Td   *big.Int
}
...
// NewChtIndexer함수는 CHT chain indexer를 생성한다	
