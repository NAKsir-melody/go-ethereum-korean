...
// Send는 메시지를 다음 사이클에 분산될 whisper 전송큐에 삽입한다.
...
// HandlePeer는 Whisper 서브프로토콜의 연결이 협상되었을때 아랫쪽의 p2p layer에 의해 호출된다
...
// runMessageLoop는 들어오는 메시지를 읽고 처리한다
...
// add는 새로운 봉투를 메시지 풀에 삽입한다.
// 또한 메시지를 적당한 시간표시고가 함께 만료풀에 삽입한다.
...
// processQueue는 메시지를 보는사람들에게 전달한다
