...
type ReceivedMessage struct {
	...
	// 토픽에 관련된 키의 keccak256hash
	EnvelopeHash common.Hash // Message envelope hash to act as a unique id
	// 단일키로서 동작할 메시지 봉투의 해시
}
...
// generateSecureRnadomData는 추가 보안이 필요할때 필요한 랜덤 데이터를 생성한다.
// 이함수의 목적은 진짜 랜덤데이터를 생성하기 어려운 sw/hw적인 버그를 방지하기 위해서 이다.
// 이함수는 특히 랜덤성보다, 모든 메시지에 단일한 논스를 가지는것이 매우 중요한 AES nonce에 활용하기 좋다
...
// nonce size should be 12 bytes (see cipher.gcmStandardNonceSize).
// decryptSymmetric은 AES-GCM-256을 이용하여 토픽키로 메시지를 복호화 한다
// 논스의 크기는 12바이트여야 한다
func (msg *ReceivedMessage) decryptSymmetric(key []byte) error {
	// symmetric messages are expected to contain the 12-byte nonce at the end of the payload
	// 대칭키 메시지는 12바이트의 논스를 페이로드끝에 포함하고 있음
...
// decryptAsymmetirc 은 개인키로 암호화된 페이로드를 복호화 한다
...
// ValidateAndParse는 메시지의 유효성을 검증하고 성공했을 경우 각 필드들을 푼다
