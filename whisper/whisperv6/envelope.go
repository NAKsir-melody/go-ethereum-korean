...
// Envelope은  Whisper를 통해 전송할 일반 텍스트 데이터 패킷을 나타내며,
// 내용은 암호화되거나 서명되지 않을 수도 있다.
...
// NewEnvelope은 만료정보와 목적지 정보를 포함하여 whisper메시지를 감싸고
// 네트워크 전파르 위해 봉투에 넣는다
...
// Seal은 데이터를 해싱하는 pow 로 요구되는 시간만큼을 소비함으로 인해 봉투를 닫는다
...
// OpenAsymmetric은 특정키로 암호화 되었을수도 있는 봉투를 복호화한다
...
// OpenSymmetric은 특정키로 암호화 되었을수도 있는 봉투를 복호화한다
...
// Open은 봉투를 해독하고, 성공할 경우 메시지 필드를 채운다 
func (e *Envelope) Open(watcher *Filter) (msg *ReceivedMessage) {
	...
	// API인터페이스는 필터가 비대칭키와 대칭키 암호화를 모두 사용하는것을 금지한다
